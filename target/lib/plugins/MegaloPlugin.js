...
class MegaloPlugin {
  ...
  apply( compiler ) {
    ...
    // generate pages
    megaloOptions.platform != 'web' && hookJSEntry( {
      rules,
      files: [ 'foo.js', 'foo.ts' ],
      entryLoader: {
        options: {},
    ...
    megaloOptions.platform != 'web' && hookAssets( {
      rules,
    ...
    megaloOptions.platform == 'web' && modifyVueLoaderOptions( rules );
    ...
    megaloOptions.platform != 'web' && lazyEmit( compiler, megaloTemplateCompiler, megaloOptions )
...
function modifyVueLoaderOptions (rules) {
  const vueLoaderRule = findRuleByFile(rules, ['foo.vue', 'foo.vue.html']);
  const vueLoader = vueLoaderRule.use.filter((rule) => {
    return rule.loader == 'vue-loader';
  });

  !vueLoader.options && (vueLoader.options = {});
  attachMultiPlatformModule(vueLoader.options);
}
